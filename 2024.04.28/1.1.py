...
def pick_resistors(nominal: int) -> dict[str, tuple] | None:
    result = dict()
    for key in nominals.keys():
        result[key] =tuple(map(lambda x: abs (nominal - x), nominals[key]))
        result[key] = tuple(filter(lambda x: min (result[key]) == abs(x - nominal), nominals[key]))
    return (result)
    if nominal not in range(100, 999):
        return None
